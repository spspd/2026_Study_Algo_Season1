...
public class 스타트택시{
	...
	static int pickPassenger() {
		...
		for(int i=0;i<p.size();i++) {
			int nr = p.get(i).startR;
			int nc = p.get(i).startC;
			
			if(dist[nr][nc]<minDist&&!p.get(i).isSelected) {
				minDist = dist[nr][nc];
				pick = p.get(i);
			}else if(dist[nr][nc]==minDist&&!p.get(i).isSelected) {
				// 열이 작은 승객
				if(nr<pick.startR) {
					pick = p.get(i);
				...
				//행이 작은 승객
				if(nr==pick.startR) {
					if(nc<pick.startC) pick = p.get(i);
				}
			}
		}
		...
		int move = arriveDist[pick.arriveR][pick.arriveC];
		
		if(oil<move) return -1;
		else {
			oil = oil - (minDist+move) + move*2; 
			taxiR = pick.arriveR;
			taxiC = pick.arriveC;
			return 1;
		}
